...
export class DaywiseComponent {
  ...
  constructor(private fb:FormBuilder,private reportServie:ReportsService) { }

  showReport()
  {
    // console.log("from daywise : ",this.dayForm.value);
    this.reportServie.getDayWiseReport(this.dayForm.value.date).subscribe(
      ...
      if(response.success)
      {
          if(response.data)
        {
          if(response.data.categoryReport && response.data.expenses){
            const {labels,dataset,expenses}= this.reportServie.structureData(response.data);
            this.labels=labels;
